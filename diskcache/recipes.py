...
class BoundedSemaphore:
    ...
    def __init__(self, cache, key, value=1, expire=None):
        self._cache = cache
        self._key = key
        self._value = value
        self._expire = expire
...
def throttle(
    ...
    def decorator(func):
        ...
        now = time_func()
        cache.set(key, (now, count), expire=expire, retry=True)
...
def barrier(cache, lock_factory, name=None, expire=None):
    ...
    def decorator(func):
        key = full_name(func) if name is None else name
        lock = lock_factory(cache, key, expire=expire)
...
# def memoize_stampede(cache, expire, name=None, typed=False, beta=1, ignore=()):
...
#             pair = timer(*args, **kwargs)
#             cache.set(key, pair, expire=expire, retry=True)
#             return pair[0]
